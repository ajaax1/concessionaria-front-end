...
import veiculoCreate from '../../services/veiculoCrud/veiculoCreate';
...
let categoria_id = ref<string>('');
let modelo = ref<string>('');
let ano = ref<string>('');
let cor = ref<string>('');
let preco = ref<string>('');
let descricao = ref<string>('');
let status = ref<'USADO' | 'NOVO' | 'SEMINOVO' | ''>('');
let quilometragem = ref<string>('');
let loading: Ref<boolean> = ref(false);
let response = null;
let showBrand = ref(false);
let showCategory = ref(false);
let showOpcional = ref(false);
...
const submit = async () => {
    loading.value = true;
    imagens_submit.value = imagens.value.map((element: any) => element.file);
    response = await veiculoCreate(payload.value, imagens_submit.value);
    if (response == true) {
