...
import opcionalUpdate from '../../services/opcionalCrud/opcionalUpdate';
import opcionalShow from '../../services/opcionalCrud/opcionalShow';
import opcional from '@/types/opcionalTypes/opcional';
...
onMounted(async () => {
    loading.value = true;
    datas.value = await opcionalShow(id);
...
const submit = async () => {
    ...
    response = await opcionalUpdate(payload.value, id);
    if (response == true) {
