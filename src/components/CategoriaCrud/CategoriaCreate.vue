...
const submit = async () => {
    ...
    response = await categoriaCreate(payload.value);
    if (response == true) {
    ...
    <div class="bg-gray-100 rounded-lg shadow-2xl h-screen flex flex-col w-full p-5">
        ...
        <form @submit.prevent="submit" class="h-full flex flex-col items-center">
            ...
            <button :disabled="loading" @click="submit"
                class="bg-sky-700 shadow-lg rounded-sm hover:brightness-75 mt-2 transition duration-300 flex justify-center items-center w-[250px] h-8">
