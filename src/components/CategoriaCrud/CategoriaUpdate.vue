...
import categoriaUpdate from '../../services/categoriaCrud/categoriaUpdate';
import categoriaShow from '../../services/categoriaCrud/categoriaShow';
import categoria from '@/types/categoriaTypes/categoria';
...
onMounted(async () => {
    loading.value = true;
    datas.value = await categoriaShow(id);
...
const submit = async () => {
    ...
    response = await categoriaUpdate(payload.value, id);
    if (response == true) {
        router.push('/admin/painel/categorias');
    }
    messages.value = {
        nome: response.nome ?? [],
    }
...
<template>
    <div class="bg-gray-100 rounded-lg shadow-2xl h-screen flex flex-col w-full p-5">
        <div class="w-full flex justify-center gap-2 items-center mb-5">
            ...
            <RouterLink to="/admin/painel/categorias"
